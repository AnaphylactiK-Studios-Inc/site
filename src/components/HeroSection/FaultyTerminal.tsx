...
vec3 getColor(vec2 p){
    ...
    // 5-tap cross (center reuses middle) instead of a full 3x3 grid:
    // halves the per-pixel digit() cost for a near-identical glow.
...
void main() {
    ...
    gl_FragColor = vec4(vec3(0.0), (1.0 - alpha) * 0.95);
}
`;
...
export default function FaultyTerminal({
  ...
  useEffect(() => {
    ...
    const renderer = new Renderer({ dpr, alpha: true, premultipliedAlpha: true });
    ...
    const update = (t: number) => {
      ...
      // Skip the heavy shader work entirely while the hero is scrolled offscreen.
